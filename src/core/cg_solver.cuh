    ...
    int num_pd_particle)
{
  ...
  {
    int bond_index = bond * num_pd_particle + p;
    element = matrix[bond_index + num_pd_particle];
...
//------------------------------------------------------------------------------------------
__global__ void initPDSolutions(real4_t* solution, int num_pd_particle)
{
    ...
    real_t pd_particle_mass)
{
  ...
  while(bond <= bond_top_index)
  {
    ...
    {
      int last_bond_particle = pd_bond_list[bond_top_index * num_pd_particle + p];
      pd_bond_list[bond_index] = last_bond_particle;
      pd_bond_list[bond_top_index * num_pd_particle + p] = q;
    ...
    if (d0 > (pd_horizon - pd_particle_radius))
    {
      vscale = 0.5 + (pd_horizon - d0) / 2.0 / pd_particle_radius;
    }
    ...
    matrixMultiplyNumber(forceDx, pd_C_times_V * PD_K / d0);
    ...
    pd_system_matrix[bond_index + num_pd_particle] = forceDx;
  ...
  force *= pd_C_times_V * PD_K;// _PDStiffness[index];
  ss0 = (10.0 - 9.0 * clockScale) * pd_original_stretch[p];
  pd_new_stretch[p] = ss0 - alpha_pr * smin;

  //    force.y -= (9.8 * pd_particle_mass);
  force.y -= (clockScale * 9.8 * pd_particle_mass);
  force.w = 0;
  pd_force[p] = force;
    ...
    int num_pd_particle)
{
  ...
  {
    int q = pd_bond_list[bond * num_pd_particle + p];

    xq = x[q];
    Apq = pd_system_matrix[(bond + 1) * num_pd_particle + p];
...
class PeridynamicsMatrix: public cusp::linear_operator<real_t, cusp::device_memory>
{
  public:
    ...
    int num_blocks_, num_threads_, num_pd_particle_;
    ...
    PeridynamicsMatrix(int* pd_bond_list,
        ...
        Mat3x3* pd_system_matrix,
        int matrix_size,
        int num_blocks, int num_threads, int num_pd_particle)
      ...
      num_pd_particle_(num_pd_particle)
  {}
